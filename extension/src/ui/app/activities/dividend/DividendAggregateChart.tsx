...
type Props = {
  items: App.Activity.DividendItem[];
  censorSensitiveData: boolean;
};

export default function DividendAggregateChart({ items, censorSensitiveData }: Props) {
  const aggregates = [];
  const amounts = [];
  const labels = [];
  ...
  const option: EchartProps["option"] = {
    tooltip: {
      ...
      formatter: ChartUtilities.makeTooltipFormatter(censorSensitiveData, items, (data) => {
        return `Amount: ${data.amount.text}<br />Aggregate: ${data.aggregate.text}`;
      }),
    },
    xAxis: {
      data: labels,
